...
class TestRefuteRound:
    """Second-pass refutation in the same warm process (R3-C4).

    같은 웜 프로세스에서의 2차 반박 검증 (R3-C4).
    """
    ...
    def _request_with_gate(
        self, host: JudgeHost, threshold: float = 0.8
    ) -> dict:
        host.ensure_started()
        assert _wait_until(lambda: host._ready)
        ours, theirs = socket.socketpair()
        try:
            ...
            return _recv_reply(theirs)
        finally:
            theirs.close()

    def test_qualifying_verdict_gets_refute_round(
        self, host: JudgeHost, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        prompts: list[str] = []
        responses = [VERDICT_JSON, '{"refuted": false, "reason": "타당"}']
        ...
        monkeypatch.setattr(JudgeHost, "_round_trip", fake_round_trip)
        reply = self._request_with_gate(host)
        assert reply["ok"] is True
        assert reply["refute"] == {"refuted": False, "reason": "타당"}
        # Two turns in the same process: judgment then refutation, and
        # the refute prompt quotes the verdict's evidence.
        # 같은 프로세스 2턴 — 판정 후 반박. 반박 프롬프트는 판정의
        # evidence 를 인용한다.
        assert len(prompts) == 2
        assert "반박하라" in prompts[1]
        assert "JWT 교체 완료" in prompts[1]

    def test_below_threshold_skips_refute(
        self, host: JudgeHost, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        prompts: list[str] = []
        ...
        def fake_round_trip(self, text: str, timeout: float) -> str:
            prompts.append(text)
            return VERDICT_JSON  # confidence 0.9

        monkeypatch.setattr(JudgeHost, "_round_trip", fake_round_trip)
        reply = self._request_with_gate(host, threshold=0.95)
        assert reply["ok"] is True
        assert "refute" not in reply
        assert len(prompts) == 1

    def test_no_gate_skips_refute(
        self, host: JudgeHost, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(
            JudgeHost, "_round_trip", lambda self, text, timeout: VERDICT_JSON
        ...
        ours, theirs = socket.socketpair()
        try:
            host.handle_request({"prompt": "질문", "session_id": "conv-1"}, ours)
            reply = _recv_reply(theirs)
        finally:
            theirs.close()
        assert "refute" not in reply

    def test_unusable_refute_answer_reports_refuted(
        self, host: JudgeHost, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        calls: list[str] = []

        def fake_round_trip(self, text: str, timeout: float) -> str | None:
            calls.append(text)
            if len(calls) == 1:
                return VERDICT_JSON
            return None  # 반박 라운드 타임아웃

        monkeypatch.setattr(JudgeHost, "_round_trip", fake_round_trip)
        reply = self._request_with_gate(host)
        # An unverified switch must not run automatically.
        # 검증되지 않은 전환을 자동 실행하면 안 된다.
        assert reply["refute"] == {
            "refuted": True,
            "reason": "refute_unavailable",
        }
