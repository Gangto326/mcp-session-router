...
Claude Code 자체 `/resume` picker로 롤오버된 이전 대화에 직접 들어가는 것을 래퍼가 막을 수는 없다. 진입 자체는 대화 로드 직후 감지되어 경고 한 줄이 표시되고, 다음 입력 때 최신 대화로 이동 여부를 묻는다. 감지는 보수적이라 — 판별이 모호한 대화(과거 버전이 남긴 교차 링크 등)에서는 경고를 생략한다. 진입한 대화가 하드 컨텍스트 한계까지 차 있으면 질문 턴 자체가 실행되지 못할 수 있다 — 이 경우 경고 한 줄만 표시되고, 다음 입력부터 롤오버가 정상 재개된다.
